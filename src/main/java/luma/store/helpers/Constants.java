package luma.store.helpers;

public class Constants {
    public static final String BROWSER = System.getProperty("browser", "chrome");
    public static final String REMOTE = System.getProperty("remote");
    public static final String LUNA_URL = "https://magento.softwaretestingboard.com/";
    public static final String CART_URL = "https://magento.softwaretestingboard.com/checkout/cart/";
    public static final String EMAIL = "testuserte5t@yandex.com";
    public static final String PASSWORD = "SeleniumFinalTask160";
    public static final String RANDOM = "Random";
    public static final String WHATS_NEW_CATEGORY = "What's New";
    public static final String WOMEN_CATEGORY = "Women";
    public static final String JACKETS_SUB_CATEGORY = "Jackets";
    public static final String TOPS_SUB_CATEGORY = "Tops";
    public static final String BOTTOMS_SUB_CATEGORY = "Bottoms";
    public static final String PANTS_SUB_CATEGORY = "Pants";
    public static final String WOMAN_JACKET_PRODUCT_NAME = "Ingrid Running Jacket";
}
